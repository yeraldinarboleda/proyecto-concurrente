...
def create_dotplot(seq1, seq2, output_dir, window_size=500, step_size=100, memory_limit=5*1024**3, max_system_memory=85):
    ...
    save_interval = 1 * 60  # 1 minuto en segundos
    image_counter = 0
    ...
    elapsed_times = []
    # last_adjust_time = 0
    # adjustment_interval = 10  # Ajustar cada 10 segundos
    ...
    for i in range(0, len1 - window_size + 1, step_size):
        for j in range(0, len2 - window_size + 1, step_size):
            ...
            #     if current_time - last_adjust_time >= adjustment_interval:
            #         print("Uso de memoria cerca del límite. Liberando memoria y ajustando parámetros.")
            #         gc.collect()  # Forzar recolección de basura
            #         # Ajustar tamaño de ventana y paso si es necesario
            ...
            if current_step % 2000 == 0:
                ...
                if current_time - last_save_time >= save_interval:
                    ...
                    util.save_performance_graphs(elapsed_times, output_dir)

                gc.collect()

    ax.set_xlabel('Secuencia 2')
    ax.set_ylabel('Secuencia 1')
    ax.set_title('Dotplot de E. coli vs Salmonella')
    plt.show()
